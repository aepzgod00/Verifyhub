...
function parseResult(input) {
  ...
  let targetStr = typeof input === "string" ? input : JSON.stringify(input);
  
  // คลีน Markdown block เผื่อ AI แนบติดมา
  targetStr = targetStr.replace(/```json|```/gi, "").trim();
  ...
  try {
    ...
    try {
      // ดักตรวจค้นหาโครงสร้างด้วย Regex แบบยืดหยุ่นสูง
      ...
      console.error("Regex parsing fails:", e);
    }
  }
  return null;
}
...
function ResultDisplay({ data, rawText }) {
  ...
  // คำนวณสถิติ
  const totalMismatch = parsed.reduce((acc, bl) => acc + (bl.items || bl.fields || []).filter(i => String(i.status).toUpperCase() === "MISMATCH").length, 0);
  const totalReview   = parsed.reduce((acc, bl) => acc + (bl.items || bl.fields || []).filter(i => String(i.status).toUpperCase() === "REVIEW").length, 0);
  const totalItems    = parsed.reduce((acc, bl) => acc + (bl.items || bl.fields || []).length, 0);
  ...
  return (
    <div style={{ marginTop: "28px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "16px", padding: "20px 24px", borderRadius: "var(--r-lg)", background: overallPass ? "var(--success-l)" : "var(--error-l)", border: `1px solid ${overallPass ? "var(--success)" : "var(--error-b)"}`, marginBottom: "24px" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: "13px", color: "var(--text-2)", marginTop: "2px" }}>
            ตรวจสอบทั้งสิ้น {totalItems} ฟิลด์ ในเอกสาร B/L จำนวน {parsed.length} ชุด
          </div>
        </div>
      ...
      {parsed.map((bl, i) => {
        const rowItems = bl.items || bl.fields || [];
        return (
          <div key={i} className="result-card" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-lg)", overflow: "hidden", marginBottom: "24px", boxShadow: "var(--shadow-sm)" }}>
            <div className="result-card-header" style={{ padding: "16px 24px", background: "var(--surface-2)", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontWeight: "600", fontSize: "15px", display: "flex", alignItems: "center", gap: "8px", color: "var(--text)" }}>
                <SvgDocument /> B/L No: <strong style={{ fontFamily: "monospace", color: "var(--primary-h)" }}>{bl.bl || bl.blNumber || `Document #${i+1}`}</strong>
              </span>
              {bl.consignee && (
                <span style={{ fontSize: "12px", color: "var(--muted)" }}>Consignee: {bl.consignee}</span>
              )}
            </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "13px" }}>
                ...
                <tbody>
                  {rowItems.map((item, idx) => {
                    const statusStr = String(item.status || "MATCH").toUpperCase();
                    const isMismatch = statusStr === "MISMATCH";
                    const isReview = statusStr === "REVIEW";
...
function AuditPage({ onBack }) {
  ...
  const runAudit = async () => {
    ...
    try {
      ...
      // เชื่อมต่อกลับเข้า Worker Endpoint ตัวหลักที่รันระบบคัดกรองข้อมูล
      ...
      if (data.result) {
        ...
        setRawText(typeof data.result === "string" ? data.result : JSON.stringify(data.result));
      } else if (data.text) {
        setRawText(data.text);
        setResult(data.text);
      } else {
        throw new Error("ระบบตอบกลับโครงสร้างข้อมูลไม่ถูกต้อง");
      }
    } catch (err) {
  ...
  return (
    <div>
      ...
      <main className="main">
        ...
        {loading && (
          <div className="loading-box" style={{ textAlign: "center", padding: "24px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-md)" }}>
            <div className="loading-spinner" style={{ margin: "0 auto 12px" }}></div>
            <div className="loading-stage" style={{ fontWeight: "600" }}>กำลังตรวจสอบโครงสร้างความถูกต้องเอกสาร...</div>
            <div className="loading-sub" style={{ fontSize: "12px", color: "var(--muted)" }}>Cross-referencing items from Seabra Server</div>
          </div>
        )}

        {(result || rawText) && !loading && <ResultDisplay data={result} rawText={rawText} />}
      </main>
    </div>
  );
}
